const ordersummary = document.querySelector(".ordersummaryopenerwrap");
ordersummary.addEventListener("click", opensummary);

let val = 1;

function opensummary() {
  if (val == 1) {
    document.querySelector(".ordsumarrow").classList.add("rotatearrow");
    document.querySelector(".summaryphone").classList.remove("hidden");
    val = val + 1;
    console.log("open");
  } else if (val == 2) {
    document.querySelector(".ordsumarrow").classList.remove("rotatearrow");
